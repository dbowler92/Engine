#include "PushConstantCube.h"

//Queue
#include "../../Engine/Graphics/CommandQueue/CommandQueue.h"

//Graphics subsystem include
#include "../../Engine/Graphics/Manager/GraphicsManager.h"

PushConstantCube::PushConstantCube()
{}

PushConstantCube::~PushConstantCube()
{}

void PushConstantCube::Shutdown()
{
	...
	pushConstantBinding.Shutdown();
}

void PushConstantCube::Init(EngineAPI::Graphics::GraphicsManager* graphicsSubsystem)
{
	...
	ib.SetResourceDebugName("Push Constant Cube Index Buffer");
	...
	*/
	...
	//Program
	//
	assert(testProgramSPIR.CreateVKShaderModule(device, SHADER_ASSETS_FOLDER"TestShaders/PushConstant-vert.spv", SHADER_STAGE_VERTEX_SHADER, "main", true));
	assert(testProgramSPIR.CreateVKShaderModule(device, SHADER_ASSETS_FOLDER"TestShaders/PushConstant-frag.spv", SHADER_STAGE_FRAGMENT_SHADER, "main", true));
	...
	//Push constant data
	//
	assert(pushConstantBinding.InitVKPushConstantBinding(device, VK_SHADER_STAGE_FRAGMENT_BIT, 0, 8));
	...
	//
	assert(graphicsPipelineLayout.InitVKGraphicsPipelineLayout(device, &descriptorSet, 1, &pushConstantBinding, 1));
...
void PushConstantCube::Update(float dt)
{
	static uint32_t colourFlag = 1;
	...
	//Update constants
	float mixValue = 0.3f;
	uint32_t updateConstData[2]; //0 => 32 bit int colour to use flag, 1 => mix (32 bit float)
	updateConstData[0] = colourFlag;
	memcpy(&updateConstData[1], (void*)&mixValue, sizeof(float));

	EngineAPI::Graphics::CommandQueue* graphicsQueue = device->GetGraphicsCommandQueueFamily()->GetCommandQueueAtIndex(0);
	assert(pushConstantBinding.UpdateVKPushConstantBindingData(graphicsQueue, &graphicsPipelineLayout, updateConstData));
}

void PushConstantCube::GenerateRenderingCommands(EngineAPI::Graphics::GraphicsManager* graphicsSubsystem)
{
